...
profile5d: build-profile $(PROFILES_DIR)
	HYPERCHESS_COMPUTE_MINUTES=$(MINUTES) HYPERCHESS_COMPUTE_CONCURRENCY=$(CONCURRENCY) \
		perf record -g -F 997 -o $(PROFILES_DIR)/perf_5d.data -- $(PROFILE_BIN) 5 3
	perf script -i $(PROFILES_DIR)/perf_5d.data | inferno-collapse-perf | \
		inferno-flamegraph --title "HyperChess 5D (depth 3)" > $(PROFILES_DIR)/flamegraph_5d.svg
	perf report -i $(PROFILES_DIR)/perf_5d.data --stdio --no-children --percent-limit 1.0

profile: profile2d profile3d profile4d profile5d
