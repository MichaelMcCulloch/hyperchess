...
fn test_u64_board_initialization() {
    ...
    board.pieces.white_occupancy.set_bit(0);
    assert!(board.pieces.white_occupancy.get_bit(0));
    assert_eq!(board.pieces.white_occupancy.count_ones(), 1);

    board.pieces.white_occupancy.clear_bit(0);
    assert!(!board.pieces.white_occupancy.get_bit(0));

    assert_eq!(board.geo.cache.index_to_coords.len(), 64);
